...
public class SimpleTextParserTest {
    ...
    public void testMaxStringLength_defaultValue() {
        ...
        // act/assert
        Assert.assertEquals(1024, p.getMaxStringLength());
    }
    ...
    public void testCharacterPosition_givenPosition() throws IOException {
        // arrange
        SimpleTextParser p = parser("abc\rdef");
    ...
    public void testHasMoreCharacters() throws IOException {
        // arrange
        SimpleTextParser empty = parser("");
        SimpleTextParser nonEmpty = parser("a");

        // act/assert
        Assert.assertFalse(empty.hasMoreCharacters());

        Assert.assertTrue(nonEmpty.hasMoreCharacters());
        assertChar('a', nonEmpty.readChar());
        Assert.assertFalse(nonEmpty.hasMoreCharacters());
    }
    ...
    public void testHasMoreCharactersOnLine() throws IOException {
        // arrange
        SimpleTextParser empty = parser("");
        SimpleTextParser singleLine = parser("a");
        SimpleTextParser multiLine = parser("a\r\nb\rc\n\n");

        // act/assert
        Assert.assertFalse(empty.hasMoreCharactersOnLine());

        Assert.assertTrue(singleLine.hasMoreCharactersOnLine());
        assertChar('a', singleLine.readChar());
        Assert.assertFalse(singleLine.hasMoreCharactersOnLine());

        Assert.assertTrue(multiLine.hasMoreCharactersOnLine());
        assertChar('a', multiLine.readChar());

        Assert.assertFalse(multiLine.hasMoreCharactersOnLine());
    ...
    public void testBasicTokenMethods() throws IOException {
        // arrange
        SimpleTextParser p = parser("abcdef\r\n\r ghi");

        // act/assert
        assertToken(p, null, -1, -1);
        Assert.assertFalse(p.hasNonEmptyToken());

        assertToken(p.next(1), "a", 1, 1);
        Assert.assertTrue(p.hasNonEmptyToken());
    ...
    public void testGetCurrentTokenAsDouble() throws IOException {
        // arrange
        SimpleTextParser p = parser("1e-4\n+5\n-4.001");

        // act/assert
        p.nextLine();
        Assert.assertEquals(1e-4, p.getCurrentTokenAsDouble(), EPS);

        p.nextLine();
        Assert.assertEquals(5.0, p.getCurrentTokenAsDouble(), EPS);

        p.nextLine();
        Assert.assertEquals(-4.001, p.getCurrentTokenAsDouble(), EPS);
    }
    ...
    public void testGetCurrentTokenAsDouble_includedNumberFormatExceptionOnFailure() throws IOException {
        ...
        try {
            ...
            Assert.assertEquals(NumberFormatException.class, exc.getCause().getClass());
        }
    }
    ...
    public void testGetCurrentTokenAsInt() throws IOException {
        // arrange
        SimpleTextParser p = parser("0\n+5\n-401");

        // act/assert
        p.nextLine();
        Assert.assertEquals(0, p.getCurrentTokenAsInt());

        p.nextLine();
        Assert.assertEquals(5, p.getCurrentTokenAsInt());

        p.nextLine();
        Assert.assertEquals(-401, p.getCurrentTokenAsInt());
    }
    ...
    public void testNextAlphanumeric() throws IOException {
        // arrange
        SimpleTextParser p = parser("a10Fd;X23456789-0\ny");
    ...
    public void testPeek_lenArg() throws IOException {
        ...
        Assert.assertEquals("bcde", p.peek(4));
        assertPosition(p, 1, 2);

        Assert.assertEquals("bcdef\r", p.peek(6));
        assertPosition(p, 1, 2);

        Assert.assertEquals("bcdef\r\n\r ghi", p.peek(100));
        assertPosition(p, 1, 2);
        ...
        Assert.assertNull(p.peek(0));
        Assert.assertNull(p.peek(100));
    }
    ...
    public void testPeek_predicateArg() throws IOException {
        ...
        // act/assert
        Assert.assertEquals("", p.peek(c -> false));
        assertPosition(p, 1, 1);

        p.readChar();

        Assert.assertEquals("bcdef", p.peek(SimpleTextParser::isAlphanumeric));
        assertPosition(p, 1, 2);
        ...
        Assert.assertNull(p.peek(c -> true));
        Assert.assertNull(p.peek(c -> false));
    }
    ...
    public void testTryMatch() throws IOException {
        ...
        Assert.assertTrue(p.tryMatch("abc"));

        Assert.assertFalse(p.tryMatch("ab"));
        Assert.assertFalse(p.tryMatch(""));
        Assert.assertFalse(p.tryMatch(null));

        Assert.assertFalse(p.tryMatch("ABC"));
        Assert.assertFalse(p.tryMatch("aBc"));

        p.next(1);
        Assert.assertTrue(p.tryMatch(null));
    }
    ...
    public void testTryMatchIgnoreCase() throws IOException {
        ...
        Assert.assertTrue(p.tryMatchIgnoreCase("abc"));
        Assert.assertTrue(p.tryMatchIgnoreCase("ABC"));
        Assert.assertTrue(p.tryMatchIgnoreCase("aBc"));
    ...
    public void testChoose() throws IOException {
        ...
        Assert.assertEquals(0, p.choose("a"));

        Assert.assertEquals(0, p.choose("a", "b", "c"));
        Assert.assertEquals(2, p.choose("c", "b", "a"));

        p.next(1);

        Assert.assertEquals(0, p.choose("b"));

        Assert.assertEquals(1, p.choose("a", "b", "c"));
        Assert.assertEquals(1, p.choose("c", "b", "a"));
    }
    ...
    public void testChooseIgnoreCase() throws IOException {
        ...
        Assert.assertEquals(0, p.chooseIgnoreCase("A"));

        Assert.assertEquals(0, p.chooseIgnoreCase("A", "b", "C"));
        Assert.assertEquals(2, p.chooseIgnoreCase("C", "b", "A"));

        p.next(1);

        Assert.assertEquals(0, p.chooseIgnoreCase("b"));

        Assert.assertEquals(1, p.chooseIgnoreCase("A", "b", "C"));
        Assert.assertEquals(1, p.chooseIgnoreCase("C", "b", "A"));
    }
    ...
    public void testTryChoose() throws IOException {
        ...
        Assert.assertEquals(0, p.tryChoose("a"));

        Assert.assertEquals(0, p.tryChoose("a", "b", "c"));
        Assert.assertEquals(2, p.tryChoose("c", "b", "a"));

        p.next(1);

        Assert.assertEquals(0, p.tryChoose("b"));

        Assert.assertEquals(1, p.tryChoose("a", "b", "c"));
        Assert.assertEquals(1, p.tryChoose("c", "b", "a"));

        Assert.assertEquals(-1, p.tryChoose("A", "B", "C"));
        Assert.assertEquals(-1, p.tryChoose());
        Assert.assertEquals(-1, p.tryChoose((String) null));
    }
    ...
    public void testTryChooseIgnoreCase() throws IOException {
        ...
        Assert.assertEquals(0, p.tryChooseIgnoreCase("a"));

        Assert.assertEquals(0, p.tryChooseIgnoreCase("A", "B", "C"));
        Assert.assertEquals(2, p.tryChooseIgnoreCase("C", "b", "A"));

        p.next(1);

        Assert.assertEquals(0, p.tryChooseIgnoreCase("B"));

        Assert.assertEquals(1, p.tryChooseIgnoreCase("a", "B", "c"));
        Assert.assertEquals(1, p.tryChooseIgnoreCase("c", "b", "a"));

        Assert.assertEquals(-1, p.tryChooseIgnoreCase("X", "Y", "Z"));
        Assert.assertEquals(-1, p.tryChooseIgnoreCase());
        Assert.assertEquals(-1, p.tryChooseIgnoreCase((String) null));
    }
    ...
    public void testUnexpectedToken() throws IOException {
        ...
        // act/assert
        Assert.assertEquals("Parsing failed at line 1, column 1: expected test but found no current token",
                p.unexpectedToken("test").getMessage());

        p.nextAlphanumeric();
        Assert.assertEquals("Parsing failed at line 1, column 1: expected test but found [abc]",
                p.unexpectedToken("test").getMessage());

        p.nextAlphanumeric();
        Assert.assertEquals("Parsing failed at line 1, column 4: expected test but found end of line",
                p.unexpectedToken("test").getMessage());

        p.discardLine();

        p.next(SimpleTextParser::isWhitespace);
        Assert.assertEquals("Parsing failed at line 2, column 1: expected test but found empty token followed by [d]",
                p.unexpectedToken("test").getMessage());

        p.next(3).next(10);
        Assert.assertEquals("Parsing failed at line 2, column 4: expected test but found end of content",
                p.unexpectedToken("test").getMessage());
    }
    ...
    public void testUnexpectedToken_causeArg() throws IOException {
        ...
        IllegalStateException exc = p.unexpectedToken("test", cause);
        Assert.assertEquals("Parsing failed at line 1, column 1: expected test but found [abc]",
                exc.getMessage());
    ...
    public void testUnexpectedToken_ioError() throws IOException {
        // arrange
        FailBuffer b = new FailBuffer(new StringReader("abc"));
        SimpleTextParser p = new SimpleTextParser(b);

        // act/assert
        b.setFail(false);
        p.next(SimpleTextParser::isDecimalPart);
        b.setFail(true);
        Assert.assertEquals("Parsing failed at line 1, column 1: expected test but found empty token",
                p.unexpectedToken("test").getMessage());
        ...
        p.nextAlphanumeric();
        b.setFail(true);
        Assert.assertEquals("Parsing failed at line 1, column 4: expected test but found no current token",
                p.unexpectedToken("test").getMessage());
    }
    ...
    public void testParseError_givenLineCol() {
        ...
        // act
        IllegalStateException exc = p.parseError(5, 6, "test message");

        Assert.assertEquals("Parsing failed at line 5, column 6: test message", exc.getMessage());
        Assert.assertNull(exc.getCause());
    }
    ...
    public void testParseError_givenLineCol_withCause() {
        ...
        // act
        IllegalStateException exc = p.parseError(5, 6, "test message", cause);
    ...
    private static void assertCharacterSequence(final SimpleTextParser parser, final String expected)
        ...
        for (int i = 0; i < expected.length(); ++i) {
            ...
            String msg = "Failed at index " + i + ":";

            Assert.assertEquals(msg, expectedChar, parser.peekChar());
            Assert.assertEquals(msg, expectedChar, parser.peekChar());

            Assert.assertTrue(parser.hasMoreCharacters());
            Assert.assertEquals(msg, expectedChar, parser.readChar());
        }

        Assert.assertFalse(parser.hasMoreCharacters());
        Assert.assertEquals(-1, parser.peekChar());
        Assert.assertEquals(-1, parser.peekChar());
        Assert.assertEquals(-1, parser.readChar());
    }

    private static void assertChar(final int expected, final int actual) {
        String expectedStr = describeChar(expected);
        String actualStr = describeChar(actual);

        Assert.assertEquals("Expected [" + expectedStr + "] but was [" + actualStr + "];",
                expected, actual);
    }

    private static void assertMatchesAll(final IntPredicate pred, final String chars) {
        for (char ch : chars.toCharArray()) {
            String msg = "Expected predicate to match [" + describeChar(ch) + "]";
            Assert.assertTrue(msg, pred.test(ch));
        }
    }

    private static void assertDoesNotMatchAny(final IntPredicate pred, final String chars) {
        for (char ch : chars.toCharArray()) {
            String msg = "Expected predicate to not match [" + describeChar(ch) + "]";
            Assert.assertFalse(msg, pred.test(ch));
        }
    }
    ...
    private static void assertPosition(final SimpleTextParser parser, final int line, final int col) {
        Assert.assertEquals("Unexpected line number", line, parser.getLineNumber());
        Assert.assertEquals("Unexpected column number", col, parser.getColumnNumber());
    }

    private static void assertToken(final SimpleTextParser parser, final String token, final int line, final int col) {
        Assert.assertEquals("Unexpected token", token, parser.getCurrentToken());
        Assert.assertEquals("Unexpected token line number", line, parser.getCurrentTokenLineNumber());
        Assert.assertEquals("Unexpected token column number", col, parser.getCurrentTokenColumnNumber());
    }
    ...
    private static final class FailBuffer extends CharReadBuffer {
        ...
        protected FailBuffer(final Reader in) {
            super(in);
        }
